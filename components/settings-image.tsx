...
export function SettingsImage() {
  ...
  // save settings + upload QR if new
  const handleSaveSettings = async () => {
    ...
    try {
      ...
      // compress + upload if new file
      if (qrFile) {
        const compressedBlob = await resizeImage(qrFile)
        const storageReference = storageRef(storage, `qr/${qrFile.name}`)
        await uploadBytes(storageReference, compressedBlob)
        finalQrUrl = await getDownloadURL(storageReference)
        setQrUrl(finalQrUrl)
      }

      // save all to Realtime DB
      const settingsRef = ref(db, "settings")
